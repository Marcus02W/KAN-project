...
def curve2coef(x_eval, y_eval, grid, k, device="cpu"):
    ...
    has_nan = torch.isnan(x_eval).any()
    print(f"x_eval contains NaN: {has_nan}")

    #torch.save(x_eval, "/content/drive/MyDrive/KAN/x_eval.pt")
    #torch.save(y_eval, "/content/drive/MyDrive/KAN/y_eval_tensor.pt")
    #torch.save(grid, "/content/drive/MyDrive/KAN/grid.pt")

    #mat = B_batch(x_eval, grid, k, device=device).permute(0, 2, 1)
    mat = B_batch(x_eval, grid, k, device=device) # (batch, in_dim, G+k)

    #torch.save(mat, "/content/drive/MyDrive/KAN/mat_tensor.pt")

    mat = mat.permute(1,0,2)[:,None,:,:].expand(in_dim, out_dim, batch, n_coef) # (in_dim, out_dim, batch, n_coef)
    # coef shape: (in_dim, outdim, G+k) 
    
    #torch.save(mat, "/content/drive/MyDrive/KAN/mat_per_tensor.pt")
    ...
    print(f"Result solution shape: {result.solution.shape}")
